...
function ShowGroupAt() {
    ShowComDiag("/common/dialog/selgroup.aspx?source=plat#atuser", "请选择需要@的部门或联系人");
}
function AddTopic()
{
...
{
    var tlp="<li title='{0}' data-uid='{2}' class='likeids_li'><a href='javascript:;'><img src='{1}' /></a></li>", a="";
    ...
    if(likeobj.length>0){ 
        ...
        tlp=tlp.replace("{0}",uname).replace("{1}",uface).replace("{2}",uid);
...
function CollFunc(obj, id)//收藏,取消收藏
{
    ...
    {
        $(obj).attr("class", "glyphicon glyphicon-heart-empty nocolled");
        PostToCS("ReColl", id, function () { });
    }
    else//加入收藏
    {
        $(obj).attr("class", "glyphicon glyphicon-heart colled");
