<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Guest_BarConfig, App_Web_iglwujjl" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<script>
    $(function () {
        ...
        var ulist=$("#BarOwner_Json_T").val();
        if(!ZL_Regex.isEmpty(ulist))
        {
            ulist=JSON.parse(ulist);
            UserFunc(ulist);
        }
    });
...
<script>
    var UserList = { list: [], $body: null };
    UserList.$body = $("#UserList_Body");
    //打开选择用户弹窗
    ...
    UserList.del = function (uid) {
        $("#tr_" + uid).remove();
        UserList.list.RemoveByID(uid, "UserID");
    }
    function UserFunc(list, select) {
        var tlp = '<tr id="tr_@UserID"><td>@UserID</td><td>@UserName</td><td><a href="javascript:;" onclick="UserList.del(@UserID);" title="删除"><i class="fa fa-remove"></a></td></tr>';
        //将两个List合并并且去除重复
        UserList.list.addAll(list, "UserID");
        var html = JsonHelper.FillItem(tlp, UserList.list, null);
        UserList.$body.html("");
        UserList.$body.html(html);
        $("#BarOwner_Hid").val(UserList.list.GetIDS("UserID"));
        CloseDiag();
    }
</script>
</asp:Content>
