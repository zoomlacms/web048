<%@ page language="C#" autoeventwireup="true" inherits="Manage_User_UserManage, App_Web_j3oqy1go" title="会员管理" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<asp:Content runat="server" ContentPlaceHolderID="head" EnableViewState="false">
<title><%=Resources.L.会员管理 %></title>
<style>.order_img {display: none;}</style>
</asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <div>
        <table id="EGV" class="table table-striped table-bordered table-hover">
            <tr>
                <td class="td_s"></td>
                <td>
                    <asp:LinkButton runat="server" ID="ID_A" CommandArgument="AscID" OnClick="OrderBtn_Click">ID<i class='glyphicon glyphicon-arrow-up order_img'></i></asp:LinkButton></td>
                <td><%=Resources.L.会员名 %></td>
                <td><%=Resources.L.真实姓名 %></td>
                <td><%=Resources.L.工号 %></td>
                <td><%=Resources.L.会员组 %></td>
                <td>
                    <asp:LinkButton runat="server" ID="Purse_A" CommandArgument="Purse" OnClick="OrderBtn_Click"><%=Resources.L.资金 %><i class='glyphicon glyphicon-arrow-up order_img'></i></asp:LinkButton></td>
                <td>
                    <asp:LinkButton runat="server" ID="Addtime_A" CommandArgument="Addtime" OnClick="OrderBtn_Click"><%=Resources.L.注册时间 %><i class='glyphicon glyphicon-arrow-up order_img'></i></asp:LinkButton></td>
                <td>
                    <asp:LinkButton runat="server" ID="point_A" CommandArgument="point" OnClick="OrderBtn_Click"><%=Resources.L.积分 %><i class='glyphicon glyphicon-arrow-up order_img'></i></asp:LinkButton></td>
                <td>
                    <asp:LinkButton runat="server" ID="LoginTimes_A" CommandArgument="LoginTimes" OnClick="OrderBtn_Click"><%=Resources.L.登录次数 %><i class='glyphicon glyphicon-arrow-up order_img'></i></asp:LinkButton></td>
                <td>
                    <asp:LinkButton runat="server" ID="AuthTime_A" CommandArgument="AscAuthTime" OnClick="OrderBtn_Click"><%=Resources.L.最后登录时间 %><i class='glyphicon glyphicon-arrow-up order_img'></i></asp:LinkButton></td>
                ...
                <ItemTemplate>
                    <tr ondblclick="location='UserInfo.aspx?id=<%# Eval("UserID") %>';">
                        ...
                        <td><a href='UserModify.aspx?UserID=<%# Eval("UserID")%>&tabs=Tabs4' class="option_style"><i class="fa fa-key" title="<%=Resources.L.权限 %>"></i><%=Resources.L.权限 %></a>
                            <a href='CertificateAuditInfo.aspx?UserID=<%# Eval("UserID")%>' class="option_style"><i class="fa fa-newspaper-o" title="<%=Resources.L.证件 %>"></i><%=Resources.L.证件 %></a>
    ...
    <div class="btn-group">
        <asp:Button ID="btnLock" runat="server" Text="<%$Resources:L,批量锁定 %>" OnClick="btnLock_Click" OnClientClick="if(!IsSelectedId()){alert('未选中任何会员');return false;}" class="btn btn-primary" />
        <asp:Button ID="btnNormal" runat="server" Text="<%$Resources:L,批量解锁 %>" OnClick="btnNormal_Click" OnClientClick="if(!IsSelectedId()){alert('未选中任何会员');return false;}" class="btn btn-primary" />
        <asp:Button ID="GroupMove" runat="server" Text="<%$Resources:L,批量移动 %>" OnClick="GroupMove_Click" OnClientClick="if(!IsSelectedId()){alert('未选中任何会员');return false;}" class="btn btn-primary" />
        <%--   <asp:Button ID="RenoVate" runat="server" Text="<%$Resources:L,刷新用户 %>" CssClass="btn btn-primary" OnClientClick="if(!IsSelectedId()){alert('未选中任何会员');return false;}" OnClick="RenoVate_Click" />--%>
        <asp:Button ID="Approve" runat="server" Text="<%$Resources:L,认证用户 %>" OnClientClick="if(!IsSelectedId()){alert('未选中任何会员');return false;}" CssClass="btn btn-primary" OnClick="Approve_Click" />
        <asp:Button ID="ApproveFailure" runat="server" Text="<%$Resources:L,取消认证 %>" OnClientClick="if(!IsSelectedId()){alert('未选中任何会员');return false;}" CssClass="btn btn-primary" OnClick="ApproveFailure_Click" />
        <asp:Button ID="btnDel" runat="server" Text="<%$Resources:L,批量删除 %>" OnClick="btnDel_Click" OnClientClick="if(!IsSelectedId()){alert('未选中任何会员');return false;}else{return confirm('不可恢复性删除数据,你确定将该数据删除吗？')}" class="btn btn-primary" />
        <asp:Button ID="Export" runat="server" Text="<%$Resources:L,Excel导出 %>" CssClass="btn btn-primary" OnClick="Export_Click" />
        <input class="btn btn-primary" type="button" onclick="inputuserinfo()" value="<%=Resources.L.Excel导入 %>" />
    </div>
...
<asp:Content runat="server" ContentPlaceHolderID="ScriptContent">
    ...
    <script type="text/javascript" src="/JS/Plugs/ZL_NavTab.js"></script>
    <script>
        $().ready(function () {
