<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Content_CssManage, App_Web_brtnwmsp" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <div class="panel panel-default" style="padding: 0px;">
        ...
        <div class="panel panel-footer" style="padding: 5px; margin: 0px;">
            <div>
                <asp:Button ID="btnCSSBackup" class="btn btn-primary" runat="server" Style="width: 110px;" Text="<%$Resources:L,备份当前风格 %>" OnClientClick="return confirm('是否创建备份？(提示：备份同名文件会覆盖！）');" OnClick="btnCSSBackup_Click" />
                <%=Resources.L.目录名称 %>：
                <asp:TextBox ID="txtForderName" class="form-control text_300" runat="server" Width="350px"></asp:TextBox>
                <asp:Button ID="btnCreateFolder" class="btn btn-primary" runat="server" Text="<%$Resources:L,创建目录 %>" OnClick="btnCreateFolder_Click" />
                <ZL:SFileUp ID="SFile_Up" runat="server" IsRelName="true" FType="All" />
                <asp:Button ID="btnTemplateUpLoad" class="btn btn-primary" runat="server" Text="<%$Resources:L,上传风格 %>" OnClientClick="return confirm('即将覆盖同名风格，是否继续？');" OnClick="btnTemplateUpLoad_Click" />
            </div>
        </div>
    </div>
