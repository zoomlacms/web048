<%@ page language="C#" autoeventwireup="true" inherits="User_Magazine_AddMagazine, App_Web_1bbopsuv" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
  ...
  <div class="user_t">
    <ZL:ExGridView ID="EGV" runat="server" Width="100%" AutoGenerateColumns="False" DataKeyNames="ID" AllowPaging="True" PageSize="10" OnPageIndexChanging="GridView1_PageIndexChanging" OnRowCommand="Lnk_Click" CssClass="border" AllowSorting="true" OnRowDataBound="GridView1_RowDataBound">
      ...
      <asp:TemplateField HeaderText="操作">
        <ItemTemplate>
          <asp:LinkButton ID="LinkButton2" runat="server" CommandName="View" CommandArgument='<%# Eval("ID") %>' CssClass="option_style"><i class="fa fa-globe" title="浏览"></i></asp:LinkButton>
          <asp:LinkButton ID="LinkButton1" runat="server" CommandName="Edit" CommandArgument='<%# Eval("ID") %>' CssClass="option_style"><i class="fa fa-pencil" title="修改"></i></asp:LinkButton>
          <asp:LinkButton ID="LinkButton3" runat="server" CommandName="Del" OnClientClick="return confirm('确定删除此杂志?');" CommandArgument='<%# Eval("ID") %>' CssClass="option_style"><i class="fa fa-trash-o" title="删除"></i>删除</asp:LinkButton>
           </ItemTemplate>
