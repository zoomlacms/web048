<%@ page language="C#" autoeventwireup="true" inherits="manage_AddCRM_CustomerList, App_Web_zqlrniuq" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <asp:Label runat="server" ID="liTitle" Visible="false"></asp:Label>
        <table class="table table-striped table-bordered table-hover" style="margin-bottom:5px;">
            <tr><td>
            选择Excel导入源：
                <ZL:FileUpload ID="fileImp" runat="server" Style="margin-bottom: 2px; float:left;"  />
            ...
            <ZL:ExRepeater ID="RPT" runat="server" PageSize="10" PagePre="<tr><td><input type='checkbox' id='chkAll'/></td><td colspan='8'><div class='text-center'>" PageEnd="</div></td></tr>">
                <ItemTemplate>
                    <tr ondblclick="getinfo(<%#Eval("Flow")%>);">
                        ...
                        <td>
                        <a href="ViewCustomer.aspx?FieldName=Person_Add&id=<%#Eval("Flow") %>&modelid=<%=ModelID %>" class="option_style"><i class="glyphicon glyphicon-eye-open"></i>预览</a> <a href="CustomerManage.aspx?ModelID=<%=ModelID %>&FieldName=Person_Add&menu=edit&id=<%#Eval("Flow") %>" class="option_style"><i class="glyphicon glyphicon-edit"></i>修改</a> <a href="?menu=delete&modelid=<%=ModelID %>&code=<%#Eval("Code") %>" onclick="return confirm('你确定要将所有选择删除吗？');"  class="option_style"><i class="glyphicon glyphicon-trash"></i>删除</a></td>
                    </tr>
                </ItemTemplate>
