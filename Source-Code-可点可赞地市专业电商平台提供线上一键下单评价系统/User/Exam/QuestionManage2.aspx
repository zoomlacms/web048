<%@ page language="C#" autoeventwireup="true" inherits="User_Exam_QuestionManage2, App_Web_1crxgxls" enableviewstatemac="false" masterpagefile="~/Common/Common.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<div class="tab-content col-lg-2 col-md-2 col-xs-2 col-sm-2 padding0">
    <div class="tab-pane active treediv" id="treePanel" style="display:none;">
        <ul>
    ...
    <div class="tab-pane treediv" id="keyPanel" style="display:none;">
...
<div class="col-lg-8 col-md-8 col-xs-8 col-sm-8">
    <div style="float:left;"><h2 style="font-family:'ZoomlaXingtiJ2';">教材版本：</h2></div>
    <ul class="sel_ul" data-type="version" id="ver_ul" style="position:relative;top:25px;">
            <asp:Repeater runat="server" ID="VersionRPT" EnableViewState="false">
                <ItemTemplate>
                    <li data-val="<%#Eval("ID") %>"><a style="font-size: 1.5em; font-weight: bold;" href="javascript:;"><%#Eval("VersionName") %></a></li>
                </ItemTemplate>
            </asp:Repeater>
        </ul>
        <table class="table table-bordered table-hover table-striped">
        <tr><td>知识点</td><td>
            <ul class="sel_ul" id="knows2_ul"></ul>
                        </td></tr>
...
<div class="col-lg-2 col-md-2 col-xs-2 col-sm-2">
    <div class="panel panel-primary" style="position:fixed;right:0px;top:100px;width:180px;">
        <div class="panel-heading">我关注的学科</div>
        <div class="panel-body">
            <ul>
                <asp:Repeater runat="server" ID="Class_RPT" EnableViewState="false">
                    <ItemTemplate><li style="float:left;margin-right:5px;margin-bottom:5px;"><%#Eval("c_classname") %></li></ItemTemplate>
                </asp:Repeater>
            </ul>
            <asp:Literal runat="server" ID="MyClass_Lit" EnableViewState="false"></asp:Literal>
            <asp:HiddenField runat="server" ID="NodeIDS_Hid" />
        </div>
        <div class="panel-footer text-center">
            <a href="/User/Info/Userbase.aspx" target="_blank"  class="btn btn-xs btn-primary">前往设置</a>
        </div>
    </div>
    <div id="basket">
        <div class="hanger-con">
            ...
            <a href="javascript:;" class="btn btn-default margin_t5 clearQid">全部清空</a>
            <a href="QuestionManage2.aspx" class="btn btn-info margin_t5" style="margin-bottom:5px;">简 洁 版</a>
        </div>
    </div>
</div>
</div>
...
<style type="text/css">
    * {font-family:'Microsoft YaHei';font-size:12px;}
    ...
    #basket{position:fixed;top:290px;width:180px;background:#02a4de url(/App_Themes/User/basketbg.jpg) no-repeat;padding-left:37px;right:0px;z-index:999;overflow:hidden;min-height:244px;}
    ...
    src: url('http://code.zoomla.cn/font/ZoomlaXingtiJ2.eot?#iefix'); /* IE9 */
    src: url('http://code.zoomla.cn/font/ZoomlaXingtiJ2.eot?#iefix') format("embedded-opentype"), /* IE6-IE8 */
    url('http://code.zoomla.cn/font/ZoomlaXingtiJ2.woff') format("woff"), /* chrome、firefox */
    url('http://code.zoomla.cn/font/ZoomlaXingtiJ2.ttf') format("truetype"), /* chrome、firefox、opera、Safari, Android, iOS 4.2+ */
    url('http://code.zoomla.cn/font/ZoomlaXingtiJ2.svg#ZoomlaXingtiJ') format("svg"); /* iOS 4.1- */
...
<script>
    ...
    function GetFilter() {
        var filter = { type: 0, diffculty: 0, nodeid: $("#NodeIDS_Hid").val(), grade: 0, keyword: "", version: 0 };
...
<script>
    $(function () {
        $("#ver_ul li").click(function () {
            $.post("/User/Exam/QuestionManage.aspx", { action: "GetKnowsByVersion", version: $(this).data("val") }, function (data) {
                if (!data || data == "") { data = []; }
                else { data = JSON.parse(data); }
                data.unshift({ k_id: "", k_name: "全部" });
                var stlp = '<li data-val="@k_id">@k_name</li>';
                var items = JsonHelper.FillItem(stlp, data, function ($item, mod) {
                    $item.click(function () {
                        $(this).addClass("active").siblings("li").removeClass("active");
                        var filter = GetFilter();
                ...
                $("#knows2_ul").html("");
                $("#knows2_ul").append(items);//
            })
        });
        var verli = $("#ver_ul li").first();
        if (verli.length > 0) {
            verli.click();
        }
    })
