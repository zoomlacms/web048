<%@ page language="C#" autoeventwireup="true" inherits="test_note, App_Web_w0avtg01" masterpagefile="~/Common/Master/Empty.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<asp:Content runat="server" ContentPlaceHolderID="head"><title>项目管理</title></asp:Content>
...
<div class="note-head">
<div class="container">
        <div class="col-lg-8 col-md-8" style="padding-top: 10px;" onclick="setProgress();">
            <span style="font-size: 18px; float: left; margin-right: 10px;">项目完成度</span>
            <div class="cp_num">
                <i ng-repeat="item in complete.nums" ng-class="complete.getclass(item);"></i>
                <strong ng-bind="complete.getcomplete()+'%'"></strong>
            </div>
            <a href="javascript:;" title="查看详情" style="font-size: 12px; text-decoration: none; color: #ff8a00; margin-left: 10px;">完成度详情 <i class="fa fa-chevron-down"></i></a>
        </div>
        <div class="col-lg-4 col-md-4 text-right">
            <%--<asp:LinkButton CssClass="btn_draft" style="margin-right:30px;" runat="server" OnClick="SaveToDraft_Btn_Click" ID="SaveToDraft_Btn" OnClientClick="return note.preSave();" title="保存草稿">保存草稿</asp:LinkButton>--%>
            <input type="button" value="预览" class="btn btn-warning" style="width: 100px;" onclick="note.save();" />
        </div>
...
<%--<img ng-show="comMod.topimg!=''" ng-src="{{comMod.topimg}}" class="note-topimg_img" style="width:100%;" />--%>
<div class="container">
<div style="position: relative; height: 500px;">
    ...
    <div class="set_title">
        <div class="set_btn" ng-hide="comMod.topimg==''">
            <a class="a_set" title="设置头图">
                <i class="fa fa-cog" style="color: #FFC65A;"></i><span>设置头图</span></a>
            <ul class="list-unstyled">
                <li><a title="重新上传头图" style="color:#fff;" onclick="selTopimg();"><i class="fa fa-image"></i><span>重新上传头图</span></a></li>
     <%--           <li><a title="重新编辑头图" style="color:#fff;" onclick="showCut();"><i class="fa fa-image"></i><span>重新编辑头图</span></a></li>--%>
            </ul>
        </div>
...
<div class="toolbar">
    <div ng-show="bar.active==item.id">
        <a href="javascript:;"><i class="fa fa-close bar-btn" style="padding:7px 8px 6px 8px;" ng-click="bar.toggle(item);"></i></a>
        ...
        <a href="javascript:;" ng-click="note.delcom(item);" ng-if="$index!=0"><i class="fa fa-remove bar-btn" style="padding:6px 8px;"></i>移除节点</a>
    </div>
    <div ng-show="bar.active!=item.id">
         <a href="javascript:;"><i class="fa fa-plus bar-btn" style="padding:7px 8px 5px 8px;" ng-click="bar.toggle(item);"></i></a>
    </div>
</div>
<div class="com-text" ng-show="item.openText">
    ...
    <div class="margin-top">
        ...
        <%--   <input type="button" value="保存" class="btn btn-info" />--%>
    </div>
</div>
...
<div class="note-music container">
    <h4 style="margin-bottom: 10px;">音乐</h4>
    <div class="addmusic">
...
<div class="sortbox" ng-show="comMod.comlist.length>1">
    <a title="展开" class="pu_items pu_open" onclick="drag.open();">
        <i class="fa fa-chevron-left"></i>
        <strong>顺序编辑器</strong>
        <span>项目文字及照片</span>
    </a>
<ul class="sortul list-unstyled">
    <li ng-repeat="item in comMod.comlist | orderBy: 'orderID'">
        <img ng-src="{{getImgByType(item)}}" /></li>
</ul>
<div id="sortlist_box" class="drag_up" style="display:none;"  ng-show="comMod.comlist.length>1">
<div class="pu_items pu_close" onclick="drag.close();">
    <i class="fa  fa-chevron-right"></i>
</div>    
<div id="drag_main" class="drag_main">
    <div class="du_top clearfix">
        <strong>拖拽调整照片或文字顺序</strong>
    </div>
...
<div id="rise_down" class="rise_down" style="display:none;">
    <a class="go_rise" title="返回顶部" onclick="Control.Scroll.ToTop();">
        <i class="fa fa-chevron-up"></i>顶部
    </a>
    <a class="go_down" title="返回底部"  onclick="Control.Scroll.ToBottom();">
        底部<i class="fa fa-chevron-down"></i>
    </a>
</div>
</div>
<div class="hidden">
...
<link href="/User/Content/Note/note.css" rel="stylesheet" />
...
<script>
    ...
    .controller("NoteCtrl", function ($scope, $compile) {
        ...
        $scope.getImgByType = function (item) {
            switch (item.type) {
                case "":
                    return "/User/Content/Note/res/word_v2.gif";
                case "image":
                    return item.content;
                case "video":
                    return "/User/Content/Note/res/video_v2.gif";
                case "para":
                    return "/User/Content/Note/res/para_v2.gif";
                default:
                    return item.type;
            }
        }
