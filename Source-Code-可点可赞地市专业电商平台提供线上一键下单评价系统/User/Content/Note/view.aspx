<%@ page language="C#" autoeventwireup="true" inherits="test_view, App_Web_w0avtg01" masterpagefile="~/Common/Master/Empty.master" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<asp:Content runat="server" ContentPlaceHolderID="head">
<title>游记预览</title><style>body{position:relative;} .note{position:static!important;}</style>
...
<div class="avatar"><a href="javascript:;"><img src="{{item.UserFace}}" err-src="/Images/userface/noface.gif"/></a></div>
<div class="content">
...
<div><tm-pagination conf="pageConf"></tm-pagination><div class="clearfix"></div></div>
<div class="note_rpy margin-top">
<textarea id="content_t" style="height: 240px;"></textarea>
<%=Call.GetUEditor("content_t",4) %>
<div class="margin-top"><input type="button" class="btn btn-info" value="发表回复" ng-click="msg.add();" /></div>
</div>
</div>
</div>
...
<h2>步骤列表</h2>
<div class="margin-top"><span><a class="note_steps_a" id="note_steps_a" href="javascript:;"><i class="glyphicon glyphicon-tasks"></i> 步骤大纲</a></span></div>
</div>
...
<ItemTemplate>
<li>
<div class="step_head"><i class="fa fa-trophy flag"></i><%#Eval("Title") %></div>
<div class="step_body"><%#Eval("Content") %></div>
</li>
</ItemTemplate>
</asp:Repeater>--%>
</ul>
...
<span id="Edit_Span" visible="false" runat="server"><a href="/User/Content/Note/Note.aspx?id=<%=Mid %>"><i class="fa fa-pencil"></i>返回编辑</a></span>
</div>
</div>
</div>
...
<script>
    ...
    function InitScoll() {
        ...
        $(window.document).scroll(function () {
            $("#note_steps ul li").each(function () {
            ...
            $("#note_steps ul li").removeClass('active');
            if (curitem != "") { $("#note_steps ul [data-id='" + curitem + "']").addClass('active'); }
        });
    }
    var scope = null, note = { Video:{
